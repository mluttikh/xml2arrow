...
| `self_closing_table_element` | a field's value is **dropped** when a different table's scope is on top of the stack when it arrives (`outer.n` is null although `<n>1</n>` is present) | declared field ownership; until then, `version: 2` rejects the field at load and `Config::lint` reports it (`FieldInsideNestedTable`) |
