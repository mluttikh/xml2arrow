...
|---|---|---|
| `xml_path:` on a table | `scope:` | unchanged |
...
- `scope:` in place of each table's `xml_path:`, and `path:` in place of each
  field's, each with the same value.
...
- a field inside the `scope` of a table nested inside its own. That table
...
2. Rename the `xml_path:` key: on a table it becomes `scope:`, on a field
   `path:`. The values are unchanged.
3. On every table, declare `row:`: the name of the repeating element, or `"."`
   when the `scope` element itself is the row, as for a header.
...
- **A field outside its table's row element.** Give the value a table of its
  own, or point the field inside the row. A value on the `scope` element
